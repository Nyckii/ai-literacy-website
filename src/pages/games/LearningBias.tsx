...
// TYPES
// ============================================================
type GamePhase = 'intro' | 'labeling' | 'training' | 'test' | 'challenge' | 'explanation' | 'complete';
...
interface ImageCard {
  ...
  type: 'cat' | 'dog';
  color: 'white' | 'black';
  correctLabel: 'cat' | 'not-cat';
}

interface TestImageCard {
  id: string;
  src: string;
  type: 'cat' | 'dog';
  color: 'white' | 'black';
...
// 10 training images, interleaved cat/dog for visual variety
...
// First test: familiar patterns → AI looks great
const TEST_IMAGES: TestImageCard[] = [
  { id: 'test-wc', src: wc6Img, type: 'cat',  color: 'white', trueLabel: 'cat',     aiPrediction: 'cat'     }, // ✅
  { id: 'test-bd', src: bd6Img, type: 'dog',  color: 'black', trueLabel: 'not-cat', aiPrediction: 'not-cat' }, // ✅
];

// Challenge: unfamiliar color combos → AI fails
const CHALLENGE_IMAGES: TestImageCard[] = [
  { id: 'chal-wd', src: wd1Img, type: 'dog', color: 'white', trueLabel: 'not-cat', aiPrediction: 'cat'     }, // ❌ white dog → Cat
  { id: 'chal-bc', src: bc1Img, type: 'cat', color: 'black', trueLabel: 'cat',     aiPrediction: 'not-cat' }, // ❌ black cat → Not Cat
];
...
const GLOBAL_STYLES = `
  ...
  .lb-fade-up   { animation: abFadeUp 0.55s ease both; }
  .lb-fade-in   { animation: abFadeIn 0.4s ease both; }
  .lb-scale-in  { animation: abScaleIn 0.45s cubic-bezier(0.34,1.56,0.64,1) both; }
  .lb-bounce-in { animation: abBounceIn 0.6s cubic-bezier(0.36,0.07,0.19,0.97) both; }
  .lb-float     { animation: abFloat 3.5s ease-in-out infinite; }
  .lb-success-glow { animation: abSuccessGlow 2.5s ease-in-out infinite; }
  .lb-shake     { animation: abShake 0.55s ease; }
  .lb-spin      { animation: abSpin 0.9s linear infinite; }
  .lb-pulse     { animation: abPulse 1.6s ease-in-out infinite; }
  ...
  .lb-btn:active:not(:disabled) { transform: translateY(0px) scale(0.97); }
  .lb-btn:disabled { opacity: 0.42; cursor: not-allowed; transform: none !important; }
  ...
  .lb-btn-success:hover:not(:disabled) {
  ...
  .lb-btn-warning:hover:not(:disabled) {
    box-shadow: 0 7px 24px rgba(245,158,11,0.5);
  }
  .lb-btn-ghost {
  ...
  .lb-step-line {
    flex: 1; height: 2px; transition: background 0.5s ease; min-width: 12px;
  }
  .lb-step-label {
  ...
  /* Photo card — draggable image tile */
  .lb-photo-card {
    border-radius: 12px;
    overflow: hidden;
    cursor: grab;
  ...
  .lb-drop-zone.drag-over-cat {
    border-color: #6366F1 !important;
    background: #EEF2FF !important;
    transform: scale(1.01);
  }
  .lb-drop-zone.drag-over-notcat {
    border-color: #F59E0B !important;
    background: #FFFBEB !important;
    transform: scale(1.01);
  }
  .lb-drop-zone.tap-active {
    border-color: #A5B4FC;
    background: #F5F3FF;
    cursor: pointer;
  }
  ...
  .lb-test-card.correct {
    border-color: #10B981;
    box-shadow: 0 0 0 4px rgba(16,185,129,0.15), 0 6px 24px rgba(0,0,0,0.07);
  }
  .lb-test-card.wrong {
    border-color: #EF4444;
    box-shadow: 0 0 0 4px rgba(239,68,68,0.18), 0 6px 24px rgba(239,68,68,0.1);
  }
  .lb-test-card .photo {
    width: 100%; aspect-ratio: 4/3; overflow: hidden; background: #E5E7EB;
  }
  .lb-test-card .photo img {
    width: 100%; height: 100%;
  ...
  .lb-test-card .label-row {
    display: flex; align-items: center; justify-content: space-between;
  }

  @media (max-width: 600px) {
    .lb-btn-xl { font-size: 16px; padding: 14px 26px; }
    .lb-btn-lg { font-size: 15px; padding: 12px 22px; }
      ...
      {LB_STEPS.map((label, i) => {
        ...
        return (
          <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1 }}>
            ...
            <span className="lb-step-label" style={{
...
// ============================================================
// PHOTO CARD — draggable image tile used in labeling
// ============================================================
function PhotoCard({
  card,
  isSelected,
  ...
  return (
    <div
      ...
      role="button"
      tabIndex={0}
      aria-label={`${card.color} ${card.type}${isSelected ? ', selected — tap a zone to place' : ', tap to select or return to unsorted'}`}
    ...
    >
      ...
      {isSelected && (
        <div style={{
          ...
          <div style={{
            background: '#4F46E5', borderRadius: 999, padding: '3px 10px',
...
// ============================================================
// DROP ZONE — sortable bucket for labeling
// ============================================================
function DropZone({
  zone,
  label,
  ...
  onDragOver: (e: React.DragEvent, zone: ZoneId) => void;
  onDragLeave: () => void;
  ...
  const hasTapSelection = selectedId !== null;

  let zoneClass = 'lb-drop-zone';
  if (isDragOver) zoneClass += zone === 'cat' ? ' drag-over-cat' : ' drag-over-notcat';
  else if (hasTapSelection) zoneClass += ' tap-active';
  ...
  return (
    <div
      ...
      onDrop={() => onDrop(zone)}
      onClick={() => hasTapSelection && onZoneClick(zone)}
      aria-label={`${label} zone${hasTapSelection ? ', tap to drop selected image here' : ', tap a placed image to return it to unsorted'}`}
    >
      <div className="lb-drop-zone-header">
        ...
        <span style={{
          ...
          background: accentColor, borderRadius: 999, padding: '2px 10px',
        }}>
          {cardsHere.length}
        </span>
      </div>
      ...
      {cardsHere.length === 0 ? (
        <div style={{
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          minHeight: 100, flexDirection: 'column', gap: 6,
        }}>
          <span style={{ fontSize: 28, opacity: 0.25 }}>{emoji}</span>
          <span style={{ fontSize: 12, color: '#9CA3AF', fontWeight: 600, textAlign: 'center' }}>
            {hasTapSelection ? 'Tap to place here' : 'Drag images here from above'}
          </span>
        </div>
      ) : (
        <div style={{
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', maxWidth: 600, width: '100%' }}>
      ...
      <div style={{ marginBottom: 20, display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap', justifyContent: 'center' }}>
        <div style={{ width: 72, height: 72, borderRadius: 16, overflow: 'hidden', boxShadow: '0 4px 16px rgba(0,0,0,0.12)', flexShrink: 0 }}>
          <img src={wc1Img} alt="white cat" style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center' }} />
        </div>
        <span style={{ fontSize: 24, color: '#9CA3AF' }}>+</span>
        <div style={{ width: 72, height: 72, borderRadius: 16, overflow: 'hidden', boxShadow: '0 4px 16px rgba(0,0,0,0.12)', flexShrink: 0 }}>
          <img src={bd1Img} alt="black dog" style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center' }} />
        </div>
        <span style={{ fontSize: 24, color: '#9CA3AF' }}>→</span>
      ...
      <h1 className="lb-fade-up" style={{
        fontSize: 'clamp(26px, 5vw, 38px)', fontWeight: 800, textAlign: 'center',
        color: '#1F2937', margin: '0 0 10px', letterSpacing: '-0.03em', lineHeight: 1.2,
      }}>
        Did the AI Really Learn Cats?
      </h1>
      <p className="lb-fade-up" style={{
      ...
      <div className="lb-card lb-fade-up" style={{ padding: '28px 32px', marginBottom: 28, animationDelay: '0.2s', width: '100%' }}>
        <p style={{ margin: '0 0 16px', color: '#374151', fontSize: 16, lineHeight: 1.65 }}>
          You're going to <strong>train an AI</strong> to recognize cats. You'll sort the training
          images, watch the AI learn, and then test it on new photos.
        </p>
      ...
      <div className="lb-fade-up" style={{ display: 'flex', gap: 10, marginBottom: 24, flexWrap: 'wrap', justifyContent: 'center', animationDelay: '0.3s' }}>
        {['🏷️ Sort training data', '🤖 Train AI', '🧪 Test on new images', '💡 Discover the bias'].map(item => (
          <span key={item} style={{
            ...
            fontSize: 13, fontWeight: 600, border: '1.5px solid #C7D2FE',
          }}>
            {item}
          </span>
        ))}
...
// ============================================================
function PhaseDragLabeling({ onTrain }: { onTrain: () => void }) {
  // placements: where each card currently lives
  ...
  const cardsIn = (zone: ZoneId) => TRAINING_IMAGES.filter(c => placements[c.id] === zone);
  ...
  const statusMsg = !allSorted
    ? `Sort all ${unsorted.length} remaining image${unsorted.length !== 1 ? 's' : ''} to continue`
    : !allCorrect
    ? 'Check your labels — something looks off'
    : '✅ All correct — ready to train!';

  const statusColor = !allSorted ? '#6B7280' : !allCorrect ? '#DC2626' : '#10B981';

  function moveCard(id: string, to: ZoneId) {
    setPlacements(prev => ({ ...prev, [id]: to }));
    setSelectedId(null);
  }
  ...
  function handleDragOver(e: React.DragEvent, zone: ZoneId) {
    e.preventDefault();
  ...
  function handleDrop(zone: ZoneId) {
    ...
    const fromZone = placements[dragId];
    if (fromZone === 'unsorted') {
      // Dragging from unsorted → place in target zone
      moveCard(dragId, zone);
    } else {
      // Dragging from a placed zone → always return to unsorted
      moveCard(dragId, 'unsorted');
    }
    setDragId(null);
  ...
  // Tap/click handlers
  // Cards in unsorted: tap to select, then tap a zone to place
  // Cards in a zone: tap once to return them to unsorted immediately
  function handleCardClick(id: string) {
    const zone = placements[id];
    if (zone !== 'unsorted') {
      // In a placed zone → return to unsorted on tap
      moveCard(id, 'unsorted');
    } else {
      // In unsorted → toggle selection
      setSelectedId(prev => prev === id ? null : id);
    }
  }
  function handleZoneClick(zone: ZoneId) {
    if (selectedId && placements[selectedId] === 'unsorted') {
      // Move selected unsorted card into this zone
      moveCard(selectedId, zone);
    } else {
      setSelectedId(null);
    }
  }
  // Click unsorted pool to place selected card back
  function handleUnsortedPoolClick() {
    if (selectedId && placements[selectedId] !== 'unsorted') moveCard(selectedId, 'unsorted');
    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', maxWidth: 860, width: '100%' }}>
      ...
      <div className="lb-fade-up" style={{ textAlign: 'center', marginBottom: 8 }}>
        ...
        <p style={{ color: '#6B7280', fontSize: 15, margin: 0 }}>
          Drag images into a zone — or tap to select, then tap a zone to place.
          Tap a <em>placed</em> image to return it to the pool.
        </p>
      </div>

      {/* Tap hint */}
      {selectedId && (
        <div className="lb-fade-in" style={{
          margin: '8px 0 4px',
          background: '#EEF2FF', border: '1.5px solid #C7D2FE',
          borderRadius: 12, padding: '8px 16px',
          fontSize: 13, color: '#4F46E5', fontWeight: 700,
        }}>
      ...
      <div
        ...
        onClick={selectedId ? handleUnsortedPoolClick : undefined}
        style={{
          width: '100%', marginTop: 16, marginBottom: 20,
          background: 'linear-gradient(135deg, #F8FAFC, #F1F5F9)',
          border: `2px solid ${selectedId && unsorted.length > 0 ? '#A5B4FC' : '#E2E8F0'}`,
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          <span style={{ fontWeight: 700, color: '#4B5563', fontSize: 14 }}>
            📦 Unsorted Images
          </span>
          <span style={{
            fontSize: 12, fontWeight: 700,
            background: unsorted.length === 0 ? '#10B981' : '#6B7280',
        ...
        {unsorted.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '16px 0', color: '#10B981', fontWeight: 700, fontSize: 14 }}>
          ...
          <div style={{
            ...
            {unsorted.map((card, i) => (
              <div key={card.id} style={{ animation: `abFadeUp 0.3s ease ${i * 0.04}s both` }}>
                <PhotoCard
                  card={card}
                  isSelected={selectedId === card.id}
      ...
      {/* Two drop zones */}
      <div
        className="lb-zones-row lb-fade-up"
        style={{ display: 'flex', gap: 14, width: '100%', marginBottom: 20, animationDelay: '0.15s' }}
      >
        <DropZone
          zone="cat" label="Cat" emoji="🐱" accentColor="#4F46E5"
          cardsHere={inCat}
          {...sharedCardProps}
        />
        <DropZone
          zone="not-cat" label="Not Cat" emoji="🐶" accentColor="#D97706"
          cardsHere={inNotCat}
          {...sharedCardProps}
        />
      </div>
      ...
      {allSorted && !allCorrect && (
        <div className="lb-fade-in" style={{
          width: '100%', marginBottom: 14, padding: '12px 18px',
          ...
          <span style={{ fontSize: 14, color: '#B91C1C', fontWeight: 600 }}>
            Some images might be in the wrong zone. Look carefully at each image and try again.
          </span>
        </div>
      )}

      {/* Status + Train button */}
      <div className="lb-fade-up" style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, animationDelay: '0.2s' }}>
        <span style={{ fontSize: 13, fontWeight: 700, color: statusColor }}>{statusMsg}</span>
        <button
          className={`lb-btn lb-btn-lg ${canTrain ? 'lb-btn-success' : 'lb-btn-ghost'}`}
          disabled={!canTrain}
          onClick={onTrain}
          style={{ minWidth: 240 }}
        >
...
// ============================================================
function PhaseTraining({ onComplete }: { onComplete: () => void }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', maxWidth: 620, width: '100%' }}>
      ...
      {/* Training image grid with scan beam */}
      <div className="lb-fade-up" style={{
        display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)',
        gap: 8, width: '100%', marginBottom: 24, animationDelay: '0.15s',
        ...
        {TRAINING_IMAGES.map((card, i) => (
          <div key={card.id} style={{
            ...
            animation: `abFadeUp 0.3s ease ${i * 0.04}s both`,
            position: 'relative',
          }}>
            <img
              src={card.src}
            ...
            <div style={{
              ...
              padding: '3px 4px',
              background: card.correctLabel === 'cat' ? 'rgba(79,70,229,0.85)' : 'rgba(217,119,6,0.85)',
              textAlign: 'center',
            }}>
              <span style={{ fontSize: 9, fontWeight: 800, color: '#fff', letterSpacing: '0.04em' }}>
                {card.correctLabel === 'cat' ? 'Cat' : 'Not Cat'}
              </span>
            </div>
          </div>
        ))}
                ...
                <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{
                    ...
                    {done ? (
                      ...
                      <div className="lb-spin" style={{ width: 14, height: 14, border: '2px solid rgba(255,255,255,0.4)', borderTopColor: 'white', borderRadius: '50%' }} />
                    ) : (
      ...
      {/* Accuracy reveal */}
      {step >= 4 && (
        <div className="lb-card lb-success-glow" style={{
          ...
          <p style={{ margin: '6px 0 0', fontSize: 13, color: '#059669' }}>
            The AI correctly classified all {TRAINING_IMAGES.length} training images!
          </p>
        </div>
      )}

      {step >= 4 && (
        <button className="lb-btn lb-btn-lg lb-btn-primary lb-bounce-in" onClick={onComplete} style={{ minWidth: 220 }}>
          🧪 Test on New Images →
        </button>
      )}
    </div>
...
// ============================================================
// SHARED: Test/Challenge image card
// ============================================================
function TestCard({
  card,
  revealed,
  ...
  let cardClass = 'lb-test-card';
  if (show) cardClass += isCorrect ? ' correct' : ' wrong';

  return (
    <div className={cardClass + (show && !isCorrect ? ' lb-shake' : '')}
      style={{ animation: `abFadeUp 0.45s ease both` }}
    >
      <div className="photo">
      ...
      <div className="labels">
        ...
        <div className="label-row">
          ...
          {show ? (
            <span className={`lb-badge ${isCorrect ? 'lb-badge-correct' : 'lb-badge-wrong'}`} style={{ fontSize: 13 }}>
...
// ============================================================
function PhaseTest({ onNext }: { onNext: () => void }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', maxWidth: 640, width: '100%' }}>
      ...
      {showSummary && (
        <div className="lb-card lb-scale-in lb-success-glow" style={{
          width: '100%', padding: '24px 28px', marginBottom: 24,
          background: 'linear-gradient(135deg, #ECFDF5, #D1FAE5)',
          ...
          <p style={{ margin: '0 0 6px', fontSize: 15, color: '#065F46', lineHeight: 1.6 }}>
            The AI correctly identified both the white cat and black dog. Looks like a success…
          </p>
          <p style={{ margin: 0, fontSize: 14, color: '#059669', fontWeight: 600 }}>
            But wait — what about images with different colors?
          </p>
        </div>
      )}

      {showSummary && (
        <button className="lb-btn lb-btn-lg lb-btn-primary lb-bounce-in" onClick={onNext} style={{ minWidth: 220 }}>
          🤔 Try the challenge →
        </button>
      )}
    </div>
...
// ============================================================
// PHASE: CHALLENGE (hidden → reveal → both wrong)
// ============================================================
function PhaseChallenge({ onNext }: { onNext: () => void }) {
  const [revealed, setRevealed] = useState(false);
  const [showSummary, setShowSummary] = useState(false);

  function handleReveal() {
    setRevealed(true);
    setTimeout(() => setShowSummary(true), 2400);
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', maxWidth: 640, width: '100%' }}>
      ...
      <p className="lb-fade-up" style={{ color: '#6B7280', fontSize: 15, marginBottom: 8, textAlign: 'center', animationDelay: '0.1s' }}>
        What do you think the AI will predict for <em>these</em> images?
      </p>

      {!revealed && (
        <div className="lb-insight lb-fade-up" style={{ width: '100%', marginBottom: 20, animationDelay: '0.15s' }}>
          <p style={{ margin: 0, fontSize: 14, color: '#374151' }}>
            These images have a <strong>different color combination</strong> than what the AI was trained on.
            Make a prediction before revealing!
          </p>
        </div>
      )}

      {/* Hidden / revealed cards */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 16, width: '100%', marginBottom: 24 }}>
        {CHALLENGE_IMAGES.map((card) => (
          <div key={card.id}>
            {revealed ? (
              <TestCard card={card} revealed={true} revealDelay={0} />
            ) : (
              /* Hidden card back */
              <div className="lb-test-card" style={{ animation: 'abFadeUp 0.45s ease both' }}>
                <div className="photo" style={{
                  ...
                  <div style={{ textAlign: 'center' }}>
                    ...
                    <span style={{ fontSize: 13, color: '#9CA3AF', fontWeight: 600 }}>Hidden image</span>
                  </div>
                </div>
                <div className="labels">
      ...
      {!revealed && (
        <button
          className="lb-btn lb-btn-lg lb-btn-warning lb-bounce-in"
          onClick={handleReveal}
          style={{ minWidth: 240 }}
        >
          👁️ Reveal Challenge Images
        </button>
      )}
      ...
      {showSummary && (
        <>
          <div className="lb-scale-in" style={{ width: '100%', marginBottom: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div style={{
              ...
              <div>
                ...
                <div style={{ fontSize: 14, color: '#B91C1C', lineHeight: 1.55 }}>
                  The AI mistook the <strong>white dog</strong> for a cat, and called the <strong>black cat</strong> "Not Cat."
                  It ignored the actual animal — and went entirely by color.
                </div>
              </div>
            </div>
            ...
            <div style={{
              ...
              <p style={{ margin: '0 0 14px', fontSize: 16, fontWeight: 800, color: '#F9FAFB', lineHeight: 1.4 }}>
                "White = Cat&nbsp;&nbsp;·&nbsp;&nbsp;Black = Not Cat"
              </p>
              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                {[
                  { color: '#F5F0E0', border: '#C4A882', label: '⬜ White', result: '→ Cat', wrong: false },
                  { color: '#232323', border: '#4A4A4A', label: '⬛ Black', result: '→ Not Cat', wrong: false },
                ].map(item => (
                  <div key={item.label} style={{
                    display: 'flex', alignItems: 'center', gap: 8,
                    background: 'rgba(255,255,255,0.07)', borderRadius: 10, padding: '8px 14px',
                  }}>
                    <div style={{ width: 16, height: 16, borderRadius: 4, background: item.color, border: `2px solid ${item.border}`, flexShrink: 0 }} />
                    <span style={{ fontSize: 13, fontWeight: 700, color: '#E5E7EB' }}>{item.label}</span>
                    <span style={{ fontSize: 13, color: '#9CA3AF' }}>{item.result}</span>
                  </div>
                ))}
              </div>
            </div>
...
// ============================================================
// PHASE: EXPLANATION (Learning Bias aha moment)
// ============================================================
function PhaseExplanation({ onNext }: { onNext: () => void }) {
  ...
  const insights = [
    ...
    { icon: '⚡', title: 'AI chose the easy path', text: 'The model learned the simplest rule that fit the data: color. Real cat features were never needed.' },
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', maxWidth: 660, width: '100%' }}>
      ...
      {/* Explanation card */}
      <div className="lb-card lb-fade-up" style={{ width: '100%', padding: '24px', marginBottom: 20, background: 'linear-gradient(135deg,#FFFBEB,#FEF3C7)', border: '2px solid #FDE68A' }}>
        <p style={{ margin: '0 0 10px', fontSize: 13, fontWeight: 700, color: '#92400E', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
          🎓 This is called Learning Bias
        </p>
        <p style={{ margin: 0, fontSize: 15, color: '#78350F', lineHeight: 1.65 }}>
          <strong>Learning bias</strong> happens when an AI model picks up a shortcut pattern instead of the true concept.
          Because the training data had a perfect color correlation, the model had no reason to look deeper —
          it achieved 100% accuracy without ever understanding what a cat actually is.
        </p>
      </div>

      {/* Visual: what the AI learned */}
      {visStep >= 1 && (
        <div className="lb-card lb-scale-in" style={{ width: '100%', padding: '20px 24px', marginBottom: 20 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 12, width: '100%', marginBottom: 24 }}>
        {insights.map((ins, i) => (
          <div key={i} className="lb-card lb-fade-up" style={{
            padding: '16px 18px',
      ...
      <button className="lb-btn lb-btn-lg lb-btn-success lb-bounce-in" onClick={onNext} style={{ minWidth: 200 }}>
        🏆 See the takeaways →
      </button>
    </div>
  );
...
function PhaseComplete({ onRestart }: { onRestart: () => void }) {
  const keyLessons = [
    { icon: '⚡', title: 'AI takes shortcuts', text: 'Models learn the easiest pattern that fits the training data — not necessarily the true concept.', color: '#4F46E5' },
    { icon: '📊', title: 'Accuracy can be deceiving', text: '100% training accuracy only means the model fit that dataset — it says nothing about real understanding.', color: '#10B981' },
    { icon: '🌈', title: 'Diversity breaks shortcuts', text: 'A training set with varied examples (black cats, white dogs) forces the model to learn real features.', color: '#F59E0B' },
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', maxWidth: 660, width: '100%' }}>
      <div style={{ textAlign: 'center', marginBottom: 28 }}>
        ...
        <p className="lb-fade-up" style={{ color: '#6B7280', fontSize: 16, margin: 0, animationDelay: '0.1s' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 14, width: '100%', marginBottom: 28 }}>
        {keyLessons.map((lesson, i) => (
          <div key={i} className="lb-card lb-card-hover lb-fade-up" style={{
            padding: '20px',
            animation: `abFadeUp 0.5s ease ${0.1 + i * 0.1}s both`,
            borderTop: `3px solid ${lesson.color}`,
          }}>
      ...
      {/* Real-world examples */}
      <div className="lb-card lb-fade-up" style={{
        width: '100%', padding: '20px 24px', marginBottom: 28,
        background: 'linear-gradient(135deg, #1F2937, #111827)',
        animationDelay: '0.5s',
      }}>
        <p style={{ margin: '0 0 12px', fontSize: 13, fontWeight: 700, color: '#9CA3AF', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {[
            { emoji: '🏥', text: 'Medical AI trained on hospital X data predicting poorly for patients at hospital Y with different demographics.' },
            { emoji: '📷', text: 'Image classifiers learning to detect "wolf" by snow in the background, not the animal\'s features.' },
            { emoji: '💼', text: 'Hiring models trained on historical data learning to associate short résumés with rejection — because junior roles happened to use brief formats.' },
            { emoji: '🌍', text: 'Translation models performing worse on low-resource languages because training data was skewed toward English and European languages.' },
          ].map((ex, i) => (
            <div key={i} style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
              <span style={{ fontSize: 16, flexShrink: 0, marginTop: 1 }}>{ex.emoji}</span>
              <span style={{ fontSize: 13, color: '#D1D5DB', lineHeight: 1.55 }}>{ex.text}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', justifyContent: 'center' }}>
        <button className="lb-btn lb-btn-lg lb-btn-primary lb-bounce-in" onClick={onRestart} style={{ minWidth: 180, animationDelay: '0.6s' }}>
          🔄 Play Again
        </button>
        <a
          href="https://en.wikipedia.org/wiki/Inductive_bias"
          target="_blank"
          rel="noopener noreferrer"
          className="lb-btn lb-btn-lg lb-btn-outline"
  ...
  return (
    ...
    >
      ...
      <div key={animKey} style={{ width: '100%', display: 'flex', justifyContent: 'center' }}>
        ...
        {phase === 'labeling' && (
          <PhaseDragLabeling onTrain={() => advance('training')} />
        )}
        {phase === 'training' && (
          <PhaseTraining onComplete={() => advance('test')} />
        )}
        {phase === 'test' && (
          <PhaseTest onNext={() => advance('challenge')} />
        )}
        {phase === 'challenge' && (
          <PhaseChallenge onNext={() => advance('explanation')} />
        )}
        {phase === 'explanation' && (
          <PhaseExplanation onNext={() => advance('complete')} />
        )}
        {phase === 'complete' && (
