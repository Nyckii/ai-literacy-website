...
import { BookOpen, Check, Eye, Heart } from '@phosphor-icons/react';
        ...
        <div className="cb-t-log">
          {msg.steps.slice(0, step).map((s, i) => (
            <div key={i} className="cb-t-step">
              <span className="cb-t-check"><Check size={14} weight="bold" /></span>
              <span>{s}</span>
            </div>
          ))}
          ...
          <div className="cb-game-footer">
            {showLikeToast && (
              <div className="cb-like-toast" role="alert">
                <Heart size={15} weight="fill" /> Like at least one post first, the AI needs your signal!
              </div>
            )}
            <p className="cb-footer-hint">
              {totalEngaged === 0 ? (
                <>
                  <Heart size={14} weight="fill" /> Like or swipe through posts, the algorithm watches everything
                </>
              ) : (
                <>
                  <Heart size={14} weight="fill" /> {likedThisRound.size} liked
                  {' · '}
                  <BookOpen size={14} weight="fill" /> {readThisRound.size} read
                  {' · '}
                  <Eye size={14} weight="fill" /> {dwelledThisRound.size} viewed, the algorithm sees it all
                </>
              )}
            </p>
            <button
